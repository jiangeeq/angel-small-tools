...
public class SysLogUtil {
    ...
    public static String getControllerMethodDescription(JoinPoint point) {
        try {
            LogSensitive sysLog = SysLogUtil.getAnnotationSysLog(point);
     ...
     */
    private static LogSensitive getAnnotationSysLog(JoinPoint point) throws ClassNotFoundException {
        // 获取连接点目标类名
        ...
        for (Method method : methods) {
            boolean isAnnotationSysLog = method.getName().equals(methodName)
                    ...
                    && method.isAnnotationPresent(LogSensitive.class);
            if (isAnnotationSysLog) {
                return method.getAnnotation(LogSensitive.class);
            }
        }
        return null;
    }
